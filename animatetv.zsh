...
if [ $# = 1 ]; then
  REFID=$1
  ASX=http://www.animate.tv`wget -q http://animate.tv/radio/details.php\?id\=${REFID} -O - --save-cookies=${TMPFILE}_cookie.txt --keep-session-cookies | nkf -w | grep play.php | head -1 | perl -nle 'm|(/play.*?player)|; print $1'`
